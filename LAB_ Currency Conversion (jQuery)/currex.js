...
$().ready(function () {
const rateTags = Object.keys(exchangeRates.rates);

$("#toCurrency").html('<option value="" disabled selected>Select currency</option>');
for (const key in allCurrencies) {
if (rateTags.includes(key)) {
$("#toCurrency").append(`<option value="${key}" >${allCurrencies[key]} (${key})</option>`);
}
};

$("#toCurrency").change(function () {
const $input = $("#usdInput").val();
const $toCurrencyKey = $("#toCurrency").val();
const convertedVal = $input * exchangeRates.rates[$toCurrencyKey];

const roundedVal = convertedVal.toFixed(2);
const truncatedVal = convertedVal.toFixed(20).slice(0, -18);
const profit = "0.00" + convertedVal.toFixed(20).slice(truncatedVal.length);

// console.log(exchangeRates.rates[$toCurrencyKey]);
...
const jsonText = $("#exchangeRates").text();
